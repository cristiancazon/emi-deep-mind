        ...
        <>
            ...
            {!isOpen && (
                <button
                    ...
                    className="lg:hidden fixed bottom-6 right-6 z-40 p-3 bg-primary text-white rounded-full shadow-lg shadow-primary/40 hover:scale-110 transition-transform"
                    title="Open Tools"
                >
            ...
            <aside
                className={`
                    fixed inset-y-0 right-0 z-50 h-full bg-surface text-foreground shadow-2xl transition-all duration-300 ease-in-out
                    lg:static lg:z-auto lg:shadow-none lg:border-l lg:border-border-theme 
                    ${isOpen ? 'translate-x-0 w-80' : 'translate-x-full w-0 lg:translate-x-0 lg:w-14'}
                `}
            >
                {/* Unified Header */}
                ...
                {/* Vertical Label When Closed (Desktop Only) */}
                {!isOpen && (
                    <div className="hidden lg:flex absolute top-20 left-0 right-0 bottom-0 justify-center py-4 cursor-pointer" onClick={() => setIsOpen(true)}>
